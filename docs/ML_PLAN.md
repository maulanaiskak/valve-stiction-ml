...
## 7. Classic detector implementation (`classic.py`)

- `ellipse_stiction_index(pv_window, op_window) -> float`: fit an ellipse to the OP-PV phase plot, return apparent-stiction width. Threshold (tuned on ISDB only, never SACAC) converts this to a binary verdict.
- `kano_pattern_check(pv_window, op_window) -> bool`: qualitative stick-then-jump pattern detector, as an independent cross-check.
- `derive_label(pv_window, op_window) -> Literal["yes", "no", "uncertain"]`: combines both; "uncertain" windows are excluded from RF training data (not imputed, not guessed).
...
1. Scaffold repo, import thesis data + build manifest (folder labels imported as sanity-check field only).
2. Implement `classic.py` (ellipse-fit primary, Kano secondary). Build the small manually-inspected test fixture first — this is the one deliberate manual-labeling step, scoped to a handful of windows, not the dataset.
3. Run the classic detector across ISDB + SACAC, inspect the derived label distribution, the "uncertain" rate, and agreement/disagreement with old folder labels. This is a checkpoint — if the classic detector's uncertain rate is very high or its behavior looks unstable, that needs to be resolved before any RF work starts.
...
- **Ellipse-fit threshold** is tuned on ISDB only; document it as a chosen constant, not a physical law — same "apparent stiction" caveat the literature itself flags.
...
- **"Uncertain" windows get dropped**, which could shrink an already-small dataset further. Watch this at milestone 3 — if too much data gets dropped, the ellipse/Kano agreement criterion may need loosening (e.g. confidence-weighted rather than hard agree/disagree).
