...
- **Secondary comparison**: `HistGradientBoostingClassifier` (see §14 for why over LightGBM), same StratifiedGroupKFold/RandomizedSearchCV setup — run in `train.py` alongside RF every time, reported in the artifact's `gbm_comparison` field. Result: GBM slightly *beats* RF on ISDB's internal CV (F1 0.801 vs. 0.782, ROC-AUC 0.950 vs. 0.944) but **loses clearly on the real held-out SACAC test** (PR-AUC 0.745 vs. 0.788, ROC-AUC 0.849 vs. 0.865, F1 0.608 vs. 0.651) — a concrete generalization-gap comparison, not just a checkbox exercise, and it actually validates keeping RF as primary rather than that being an unexamined assumption from §1.
...
- **Model artifact**: joblib bundle with `{model, feature_names, window_size, normalization, label_source, max_windows_per_loop, classic_detector_threshold, predict_threshold, sklearn_version, git_commit_hash, rf_best_params, rf_cv_metrics, rf_cv_sanity_check_agreement, rf_test_metrics, rf_test_sanity_check_agreement, gbm_comparison, trained_at}` (`gbm_comparison` holds the same shape of results for the secondary model) — self-describing, so a deployed model can't silently drift out of sync with what it was trained on (unlike the thesis's `subscribe.py`, where the feature list lived separately from the model file). `inference.py`'s `load_artifact`/`predict_window` is the one sanctioned way to consume it — raw window in, `{"label", "probability"}` out, re-deriving normalization and feature selection internally rather than trusting a caller to get that order right.
...
- **Baseline results** (`python -m valve_stiction_ml.train`, best params `n_estimators=300, min_samples_leaf=4, max_features='sqrt', max_depth=20`; after §4.1's SACAC additions):
  ...
  Precision and recall trade off differently between ISDB-CV and SACAC — SACAC has far fewer false positives (15) but misses more true positives (89 FN). That's a real, honestly-reported generalization gap between two independently-sourced benchmark corpora, consistent with what §3 already set as the expectation (this isn't a SOTA claim, and even the classic detector it's approximating isn't perfect ground truth). (Before the §4.1 SACAC additions: 0.860/0.772 — the extra data moved things slightly, not dramatically, which is the honest expectation for +15 net confident windows on a ~774-window test set.)

  **Threshold tuning — tried, made things worse, reverted.** Speculated the 0.5 decision threshold (not the model's discrimination) was driving the ISDB-vs-SACAC precision/recall trade-off, so tested it: searched for the F1-maximizing threshold on ISDB out-of-fold predictions only (`find_best_threshold`, `evaluate.py`, never shown SACAC), found 0.515 — barely different from 0.5, since `class_weight='balanced'` already keeps RF reasonably calibrated. Applied to SACAC anyway to check: **F1 dropped from 0.651 to 0.628**, worse, not better. The speculation didn't hold up under actual testing — the precision/recall shift is a genuine property of the model's discrimination differing across the two domains (17.5% vs. 23.6% positive rate), not a threshold artifact a different cutoff would fix. Kept `predict_threshold: 0.5` in the deployed artifact; the tuned exploration is preserved in `rf_tuned` for the record, same pattern as the per-loop capping investigation — measure before deploying a "fix," don't assume it helps.
...
8. ✅ Gradient-boosting comparison — GBM beats RF on ISDB CV but loses on SACAC (see §9); kept RF as primary, now with concrete evidence rather than an unexamined assumption.
...
- **Per-loop window imbalance — investigated and empirically resolved (kept uncapped).** Checked the actual ISDB training set: top 5 of 60 loops contribute 72% of all windows (top 3, all "BAS", contribute 62% — `BAS8` alone gives 345 windows vs. 1-2 from the smallest loops). `StratifiedGroupKFold` already prevents this leaking across train/test, but not the training-data representativeness concern. Built `cap_windows_per_loop` (`dataset.py`, tested) and tried capping at 30/50/100/150 windows per loop, comparing each against no cap on the real held-out SACAC test. **Every capped variant scored worse than no cap** (best capped: ROC-AUC 0.859 / PR-AUC 0.765 at cap=150; uncapped: 0.865 / 0.788). Likely cause: this dataset is already small, so discarding up to 63% of ISDB's windows (cap=30) costs more than the fix gains — and since the dominant loops are all "no", capping them hard pushes the training positive rate to 40% vs. SACAC's actual 23.6%, moving the training distribution's class balance *further* from the test distribution's, not closer. Kept `max_windows_per_loop: null` (disabled) in `configs/default.yaml`, with the function and reasoning documented there and available if the dataset grows enough that this trade-off flips.
...
- **Horch's method: checked feasibility, not implemented.** §3 flagged it as "cheap to add later as an extra signal for flow loops if the manifest shows enough of them." Checked: SACAC's filenames do encode loop type (`<category>-<letter>-<industry>-<author>.csv`) and flow ("F") is actually the largest group there — 22 of 44 files. But **ISDB — the training set — has no such convention at all**, just industry prefixes (`CHEM`, `BAS`, `PAP`, ...) with no loop-type indicator, and no other metadata source available to determine which ISDB loops are flow-type without external domain research this project doesn't have access to. That means Horch's method could only ever be applied to a slice of SACAC (the untouched test set) — it can't improve training labels or the "uncertain" rate, which was the actual motivation for considering it. Not implemented: the effort (cross-correlation phase-lag detection, calibrating an even/odd-symmetry threshold) isn't justified for a supplementary check on ~22 test files that doesn't touch the training pipeline's real bottleneck. Same category of decision as DAMADICS/GIMSCOP in §4.1 — investigated and declined with reasoning, not an oversight.
- ✅ **License**: MIT (`LICENSE`).
...
## 15. Synthetic-data augmentation (fixing the live-streaming failure)

**The problem, quantified.** valve-stiction-pipeline's `STREAMING_EVALUATION.md` ran this model live against `valve-stiction-simulator`'s synthetic signal (267 streaming windows, 3 sensors with real stiction injected, 3 without) and found the RF predicted "yes" for *every single window* — ROC-AUC 0.079, worse than random, despite ROC-AUC 0.865 on real held-out SACAC data. Verified this wasn't a plumbing bug (feature extraction goes through the exact same code path the model trains with). It's a genuine train/serve distribution shift: the model had only ever seen real industrial recordings (ISDB/SACAC), never a scripted triangle-wave-OP + stick-slip-valve signal.

**Fix: domain randomization, not fitting to the exact test signal.** `synthetic.py` ports the same signal model `valve-stiction-simulator`'s `domain/simulator.py` uses (not imported cross-repo — it's ~20 lines) and randomizes it across a *range* of period/amplitude/stick-band/noise/center combinations (`sample_configs`), producing many different synthetic signals rather than one fixed config. `build_synthetic_features.py` generates two **disjoint** corpora from non-overlapping seed ranges: 250 configs × 5 windows (1250 windows) for training, 20 configs × 5 windows (100 windows) held out and never trained on — same discipline as SACAC, just for the synthetic domain. Ground truth here is exact (the generator's own `stiction_enabled` flag), unlike ISDB/SACAC where the classic detector has to serve as a pseudo-label source in the absence of any true label.

`train.py --include-synthetic` adds the synthetic training corpus to ISDB (SACAC stays the untouched real test set either way) and always reports the synthetic held-out set's metrics, whether or not synthetic data was trained on — that's the number that actually answers "does this generalize to a live stream," which SACAC (real, but not exercised as a stream) can't answer.

**Results, iteration 1** (RF, `n_estimators=500, max_features=log2, max_depth=20`, chosen by the same `RandomizedSearchCV` + `StratifiedGroupKFold` process as before; `sample_configs` sampling `stick_band`/`noise_std` independently of `amplitude`):

| | SACAC (real, untouched) | Synthetic held-out (disjoint configs) | Live stream (valve-stiction-pipeline's exact default config, not part of either training or held-out set) |
|---|---|---|---|
| Before (ISDB only) | ROC-AUC 0.865 / F1 0.651 | ROC-AUC 0.273 | **ROC-AUC 0.079**, every window predicted "yes" |
| After (+ synthetic, @ 0.5) | ROC-AUC 0.864 / F1 0.607 | ROC-AUC 0.938 | ROC-AUC 0.997, but only 70% accuracy — good ranking, threshold miscalibrated |
| After (+ synthetic, @ tuned 0.555) | ROC-AUC 0.864 / F1 0.578 | ROC-AUC 0.938 / F1 0.879 | 87% accuracy, F1 0.885, recall 100% |

**A real bug the test suite caught, not a guess.** `tests/test_synthetic.py::test_stiction_configs_are_detected_by_classic_detector` failed on the first attempt: only 11/20 random configs' classic-detector verdict agreed with their injected ground truth. Root cause, found by inspection: sampling `stick_band` and `noise_std` *independently* of `amplitude` let through configs where `amplitude` barely exceeded `stick_band` (near-continuous slipping — doesn't visually or statistically resemble stiction even though `stiction_enabled=True`) or where `noise_std` was large enough relative to `stick_band` to mask the sticking behavior entirely. These configs had a *correct* ground-truth label but a signal that didn't actually exhibit the labeled behavior — a real label-quality bug in the synthetic generator, not a detector failure, and it would have quietly taught the model to associate the *wrong* features with "stiction" for a meaningful fraction of the added training data.

**Fix: sample `stick_band` and `noise_std` as fractions of `amplitude`** (`stick_band ∈ [0.25, 0.45] × amplitude`, `noise_std ∈ [0.01, 0.05] × amplitude`), the same well-behaved regime valve-stiction-simulator's one hand-picked default config already sits in (`stick_band` is 35% of `amplitude`, `noise` is 2%). Also capped `period_samples` at 80 (window size is 100 everywhere in this project) so every window contains a full oscillation cycle — ML_PLAN.md §13 already found real evidence that `kano_pattern_check`'s majority-vote logic needs a complete cycle. With this fix, the same test passes cleanly (20/20 configs agree with ground truth).

**Results, iteration 2** (regenerated synthetic data with the fixed sampling, retrained):

| | SACAC (real, untouched) | Synthetic held-out (disjoint configs) | Live stream (same default config) |
|---|---|---|---|
| Iteration 1 (@ tuned 0.555) | ROC-AUC 0.864 / F1 0.578 | ROC-AUC 0.938 / F1 0.879 | 87% accuracy, F1 0.885 |
| **Iteration 2 (@ tuned 0.575)** | ROC-AUC 0.865 / F1 0.596 | **ROC-AUC 1.000 / F1 1.000** | **99.6% accuracy, F1 0.996, AUC 0.9998** (255 live-streamed windows; 1 false positive total) |

Four findings, not one:

1. **Augmentation closed the generalization gap** without materially hurting real-data performance — SACAC ROC-AUC is essentially unchanged (0.865→0.865 in the final iteration); the model didn't trade real-world accuracy for synthetic-world accuracy, it gained the latter almost for free.
2. **Ranking and calibration are different problems.** Even in iteration 1, the live stream's ROC-AUC (0.997, near-perfect) and its accuracy at the default 0.5 threshold (70%) told two different stories — the model ranked windows correctly almost every time, but the probability mass for "healthy" windows sat systematically above 0.5. Calibration failure, not discrimination failure — the fix (moving the decision threshold) is correspondingly different from retraining.
3. **Synthetic data quality mattered more than quantity.** Iteration 1 used 250 configs with an unconstrained parameter regime and topped out around 87% live-streaming accuracy; iteration 2 used the *same* 250-config volume with a physically-motivated constraint (stick_band/noise scaled to amplitude, matching how a real sticky valve actually behaves) and reached 99.6%. The unit test that caught the regime problem was as load-bearing as the retraining itself.
4. **The threshold fix had to stay honest, in both iterations.** The deployed `predict_threshold` (0.575 in the final model) was chosen from cross-validated out-of-fold predictions on the *training* data (ISDB + synthetic-train) only — never from the synthetic held-out set, and never from the live streaming numbers above, which were computed only *after* the threshold was already fixed. See valve-stiction-pipeline's `STREAMING_EVALUATION.md` for the full live-streaming writeup.

**What this doesn't claim**: a materially different live signal than either the training or held-out synthetic ranges could still expose a similar gap again — domain randomization narrows a distribution-shift problem for the family of signals it covers, it doesn't eliminate the general risk of train/serve mismatch. That's exactly why the system design (`HLD.md`'s NFR-1) never lets the RF model be the *only* answer — the classic detector runs unconditionally alongside it regardless of how good the RF gets.

**Checked directly, not assumed: is this overfitting?** Two different questions, checked separately.

- *Train/validation leakage* — no. `StratifiedGroupKFold` groups by config ID (`loop_id`), so all 5 windows from one synthetic config always land in the same CV fold; no near-duplicate window leaks across train/validation.
- *Does it generalize outside the trained parameter ranges, or did it just memorize this narrow regime* — genuinely tested with 40 configs deliberately sampled **outside** `sample_configs`' ranges on every axis at once (amplitude 40–90 vs. trained 10–40, period 85–100 vs. capped at 80, stick_band/noise fractions outside the trained 0.25–0.45 / 0.01–0.05, center 100–200 vs. trained 20–80). Result: **accuracy 42.5%, F1 0.303, AUC 0.474 — chance level.** The model did not learn a distribution-invariant concept of "stiction"; it learned to recognize signals resembling what it was shown (real ISDB/SACAC, plus this specific synthetic family), and that recognition doesn't transfer to a meaningfully different signal shape. This is not the same failure as the original bug (which was total, AUC 0.079, on the *exact* deployed signal) — it's a narrower, expected boundary of a data-driven model trained on a finite distribution, not a defect introduced by the fix. Also worth naming honestly: after the iteration-2 parameter tightening, the synthetic held-out set's perfect 1.000 F1/AUC stopped being a meaningful stress test in its own right — the regime became narrow and well-separated by construction, so that particular number shouldn't be read as "the model generalizes broadly." The OOD check above is the one that actually answers that question, and the answer is no, not broadly — only within the family of signals it was shown. This is the concrete reason `HLD.md`'s NFR-1 (classic detector runs unconditionally, never gated on the RF) isn't a hedge against a hypothetical: the RF's useful range has a real, measured edge.
